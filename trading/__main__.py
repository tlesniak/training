...
from trading.helpers.dataframe_transformations import report_generation
from trading.helpers.utils import report_csv_save
...
def main():
    ...
    try:
        logger = get_timed_rotating_log("bitcoin_reporting.log")
        spark_session = create_spark_session()

        logger.info("Starting preparaion of report")

        params = (
            parse_prompt_paramters()
        )  # calculate report parameters return 3 arguemtns
        ...
        logger.error(e)
        logger.error("Error in trading application !!!")
